...
FIXED_DROPSHIP_LABEL = "هلال بابل"
...
class SaleOrderLineAutomation(models.Model):
    ...
    x_studio_po_price = fields.Float(
        string="PO Price",
        help=(
            "Optional purchase price per unit. When set, it overrides the "
            "automatic purchase price for a dropship purchase order line."
        ),
    )
...
class SaleOrderAutomation(models.Model):
    ...
    qss_dropship_label = fields.Char(
        string="Dropshipping",
        compute='_compute_qss_dropship_label',
    )

    def _compute_qss_dropship_label(self):
        for order in self:
            # Management requested a fixed smart-button value/name.
            order.qss_dropship_label = FIXED_DROPSHIP_LABEL

    def action_open_qss_dropship(self):
        self.ensure_one()
    ...
    def _qss_prepare_before_confirmation(self):
        self.ensure_one()
        if self.state not in ('draft', 'sent'):
            return
        # This is the exact same calculation used by the dedicated button.
        self.action_calculate_net_weight()

    def action_confirm(self):
        """Confirm SO and run the management-required full cycle."""
        orders_to_confirm = self.filtered(lambda o: o.state in ('draft', 'sent'))
        for order in orders_to_confirm:
            order._qss_prepare_before_confirmation()

        result = super().action_confirm()

        # After standard confirmation, Odoo has created the related stock and
        # dropship procurement documents, so the remaining automation can work
        # against real records.
        for order in orders_to_confirm:
            order._qss_finish_full_cycle()
        return result

    def action_run_full_cycle(self):
        """Explicit action retained for server actions / old custom buttons."""
        for order in self:
    ...
    def _qss_get_purchase_orders(self):
        self.ensure_one()
        po_lines = self.env['purchase.order.line'].search([
            ('sale_line_id', 'in', self.order_line.ids),
        ])
    ...
    def _qss_update_purchase_orders(self, purchase_orders):
        """Synchronise PO quantities/prices before PO confirmation."""
        for po in purchase_orders:
            # A previously confirmed PO is not silently rewritten: this method
            # is intended to prepare the RFQ created by SO confirmation.
            ...
            for po_line in po.order_line.filtered(
                ...
                if 'mq_bundle_qty' in po_line._fields:
                    vals['mq_bundle_qty'] = sale_line.mq_bundle_qty
                if sale_line.x_studio_po_price > 0:
                    vals['price_unit'] = sale_line.x_studio_po_price
                po_line.write(vals)

    def _qss_link_and_confirm_purchase_orders(self, purchase_orders):
        for po in purchase_orders:
            if po.partner_id and 'requisition_id' in po._fields and not po.requisition_id:
    ...
    def _qss_set_picking_done_quantities(self, picking):
        """Set done quantities from the already-finalised SO/PO line quantities."""
        for move in picking.move_ids.filtered(lambda m: m.state not in ('done', 'cancel')):
            source_qty = False
            if move.sale_line_id:
                source_qty = move.sale_line_id.product_uom_qty
            elif getattr(move, 'purchase_line_id', False):
                source_qty = move.purchase_line_id.product_qty

            if source_qty is None or source_qty is False:
                source_qty = move.product_uom_qty
    ...
    def _qss_validate_pickings(self, purchase_orders):
        pickings = self.picking_ids | purchase_orders.mapped('picking_ids')
        for picking in pickings.sorted('id'):
            ...
            self._qss_set_picking_done_quantities(picking)
            ...
            # Some Odoo flows still return a backorder wizard despite context.
            if isinstance(result, dict) and result.get('res_model') == 'stock.backorder.confirmation':
                wizard = self.env['stock.backorder.confirmation'].browse(result.get('res_id'))
                if wizard.exists():
                    wizard.process_cancel_backorder()

    def _qss_create_and_post_customer_invoice(self):
        self.ensure_one()
    ...
    def _qss_create_and_post_vendor_bills(self, purchase_orders):
        for po in purchase_orders:
            draft_bills = po.invoice_ids.filtered(lambda bill: bill.state == 'draft')
            if draft_bills:
            ...
            bills = po.invoice_ids.filtered(lambda bill: bill.state == 'draft')
            if bills:
                bills.action_post()

    def _qss_finish_full_cycle(self):
        """Complete the non-standard post-confirmation steps exactly once."""
        self.ensure_one()
        if self.state in ('draft', 'sent'):
            return self.action_confirm()

        # Keep shipping fee correct even when the order was already confirmed.
        self._qss_sync_shipping_fee_line()

        purchase_orders = self._qss_get_purchase_orders()
        self._qss_update_purchase_orders(purchase_orders)
        self._qss_link_and_confirm_purchase_orders(purchase_orders)
        self._qss_validate_pickings(purchase_orders)
        self._qss_create_and_post_customer_invoice()
        self._qss_create_and_post_vendor_bills(purchase_orders)
        return True
