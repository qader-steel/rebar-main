...
## Update 19.0.1.2.0 - Bundle Qty + "Net weight (Copy)" fixes

* **Bundle Qty is now a real manual entry on every Sale Order line.**
  The previous code snapped `mq_bundle_qty` back to the line quantity on
  *every* onchange whenever the product was not flagged "Bundle Weight"
  (the default for all products) - so typing a value fired the onchange
  which instantly overwrote it again, and the column appeared frozen at
  `1`. That snap-back is removed from all three onchange methods in
  `models/sale_order.py`. Behaviour now:
  * product **flagged "Bundle Weight"** → the bundle count still drives
    the line quantity through `mq_bundle_multiplier` (unchanged);
  * **any other product** → whatever the user types is kept verbatim,
    nothing overwrites it;
  * the net-weight distribution (`_apply_net_weight_distribution`) now
    changes quantity/weight **only** and never touches `mq_bundle_qty`;
  * order-level **Total Bundle Qty** (`mq_total_bundle_qty`) is, as
    before, the stored sum of the per-line manual values.
* ⚠ **`models/purchase_order.py` still contains the identical
  snap-back** (lines ~34/41/49) because the request covered the Sale
  Order only. If Bundle Qty should be manually editable on Purchase
  Order lines too, the same three edits need applying there.
* **Migration `migrations/19.0.1.2.0/post-migration.py`** cleans up the
  leftover Odoo Studio field `x_studio_total_net_weight` (label
  "Net weight (Copy)") that this module used to collide with: it logs
  every `sale.order` field whose label contains "(Copy)", strips the
  field out of any `ir.ui.view` that still references it (Studio
  customisation views included), then deletes the field - falling back
  to renaming it if deletion is refused. Any failure inside the script
  is logged only and never fails the upgrade.
